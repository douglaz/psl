...
**Closed Predicate:** A predicate that is read-only. The only time a ground atom will be loaded as a random variable atom is when it is stored in the database's write partition and its predicate is _not_ specified as closed. Whether a predicate is open or closed is specific to each database.
